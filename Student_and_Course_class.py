...
class Course:
    ...
    def displayCourse(self, index):
        course = self.courses_df.iloc[index] # iloc == integer location
    ...
    def searchCourse(self, course):
        ...
        for index in range(len(self.courses_df)):
            course_name = self.courses_df.loc[index, 'courseName'].lower()  # dataframe.loc[row, column] syntax
  ...
  # function to edit/update student information
    ...
    def updateStudent(self, value, row, columnIndex, newValue):
        ...
        if columnIndex == 0:  # Student name column
            ...
            self.student_df.to_csv('student_info.csv', index=False)
            print(
                f"Student ID '{value}' updated to '{newValue}' successfully.\n")
    ...
    def updateStudentCourse(self, old_course_code, new_course_code):
        self.student_df.loc[self.student_df['course'] ==
                            old_course_code, 'course'] = new_course_code
        self.student_df.to_csv('student_info.csv', index=False)
        print(
            f"Updated course for students with course code '{old_course_code}' to '{new_course_code}'.")
